...
x0_vec = linspace(-20,20,N);            % x value matrix
y0_vec = linspace(-3,3,N);              % y value matrix

NT = 1024;                           % number of times
A = 2.5;                     % control parameter
B = 0.175;                    % inital value of x
...
for k = 1:N                    %Loop to cycle through y values
    ...
    for kk = 1:N                %Loop to cycle through every x value for specific y value
        ...
        for j = 1:NT                        % Loop to determine dynamics at each x value
            x(j+1) =  y(j);                      %x(2) = y(1)
            y(j+1) = -B*x(j) + A*y(j) - y(j)^3; %y(2) = function of x(1) and y(1)
